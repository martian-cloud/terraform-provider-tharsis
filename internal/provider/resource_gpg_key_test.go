...
`
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Attempt to update with a slightly corrupted configuration, which should fail.
			{
				Config:      testGPGKeyConfigurationCorrupt(),
				ExpectError: regexp.MustCompile("A GPG key cannot be updated"),
			},
...
// This function is the same as testGPGKeyConfigurationCreate except for a string replace near the end.
func testGPGKeyConfigurationCorrupt() string {
	createASCIIArmor := `-----BEGIN PGP PUBLIC KEY BLOCK-----
...
resource "tharsis_gpg_key" "tgk" {
	...
	`, createRootGroup(), strings.Replace(createASCIIArmor, "X", "Y", -1))
}
