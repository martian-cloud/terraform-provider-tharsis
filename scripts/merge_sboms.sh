...
find . -name "gl-sbom-*.cdx.json" -exec cyclonedx merge --output-file gl-sbom-all.cdx.json $ARGS --input-files "{}" +
...
jq -c --arg manufacture_name "Infor" \
    ...
    .metadata.component |= if $purl != "" then .purl = $purl else . end |
    .metadata.component |= if $cpe != "" then .cpe = $cpe else . end |
